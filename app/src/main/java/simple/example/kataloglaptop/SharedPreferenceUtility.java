...
public class SharedPreferenceUtility {
     ...
     */
    public static List<DaftarLaptop> getAllMenuRestoran(Context ctx) {
        String jsonString = getSharedPref(ctx).getString(FAVOR_KEY, null);
        List<DaftarLaptop> trs = new ArrayList<DaftarLaptop> ();
        if (jsonString != null) {
            ...
            try {
                ...
                for (int i=0;i<jsonArray.length();i++) {
                    ...
                    trs.add(DaftarLaptop.fromJSONObject(obj));
                }
    ...
    /*
        ...
        Perhatikan bahwa data disimpan dalam format JSON String
     */
    private static void saveAllMenuRestoran(Context ctx, List<DaftarLaptop> trs) {
        List<JSONObject> jsonTrs = new ArrayList<JSONObject>();
        JSONArray jsonArr = new JSONArray();
        for (DaftarLaptop tr : trs) {
     ...
     */
    public static void addMenuRestoran(Context ctx, DaftarLaptop tr) {
        List<DaftarLaptop> trs = getAllMenuRestoran(ctx);
        trs.add(tr);
        saveAllMenuRestoran(ctx,trs);
    }

    public static void editFavorite(Context ctx, DaftarLaptop tr) {
        List<DaftarLaptop> trs = getAllMenuRestoran(ctx);
        for (DaftarLaptop tre:trs) {
